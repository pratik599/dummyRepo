...
<table align="center">
	<tr height="40px">
		<td>
			...
			<input type="text" class="form-control-sm" id="year" name="coa_year">
		</td>
		<td>
			<label for="sem">Choose semester:</label>
			<select class="form-control-lg" id="sem" name="coa_sem"> 
				<option>--</option>
	...
	<tr height="40px">
		<td>
			...
			<input type="text" class="form-control-lg" id="act" name="coa_act">
		</td>
		
		<td>
			<label for="actp">Activity Performance:</label>
			<textarea class="form-control-lg" rows="2" id="actp" name="coa_acpt"></textarea>
		</td>
	</tr>
	<tr height="40px" align="center">
		<td colspan="2">
			<input type="submit" name="coa_submit" id="button" value="SUBMIT" onclick="loadDoc()" class="btn btn-info"></input>
		</td>
	</tr>
...
if(isset($_POST['coa_submit']))  
{
	$coa_year=$_POST['coa_year'];
	$coa_sem=$_POST['coa_sem'];
	$coa_act=$_POST['coa_act'];
	$coa_acpt=$_POST['coa_acpt'];
	switch(isset($_POST["coa_submit"]))
	{
		case (empty($coa_year)):
	...
	default:
	$query="INSERT INTO co_curricular_activities(student_id,Semester,Activities,Activity_performance,year)
	 VALUES('$aca_s_id','$coa_sem','$coa_act','$coa_acpt','$coa_year')";
	  $result = $conn->query($query);
	  if($result === TRUE )
		{
